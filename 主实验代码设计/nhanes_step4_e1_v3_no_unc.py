# =============================================================================
# nhanes_step4_e1_v3_no_unc.py
# 【E1-V3 消融实验：无不确定性传播 (No Uncertainty Propagation)】
#
# 目的：
#   Sub 层不传递 Global_unc 给主层，主层退回 3 维输入 (Sub1_mid, Sub2_mid, Sub3_mid)。
#   借此证明 Global_unc 传播是系统性捕捉高风险模糊样本的关键。
# =============================================================================
...
# 输出文件（自动存入子目录）
PARAMS_OUTPUT = os.path.join(OUTPUT_DIR, "e1_v3_params.npy")
DIM_INFO_OUTPUT = os.path.join(OUTPUT_DIR, "e1_v3_dim_info.npy")
METRICS_OUTPUT = os.path.join(OUTPUT_DIR, "step4_e1_v3_test_metrics.csv")
...
class EvolvableER_BRB:
    ...
    def _build_main_df(s1_mid, s2_mid, s3_mid, s1_unc, s2_unc, s3_unc) -> pd.DataFrame:
        # ★ E1-V3 核心修改：不再将 unc 传递给主层 dataframe ★
        return pd.DataFrame({
            "Sub1_mid": s1_mid,
            "Sub2_mid": s2_mid,
            "Sub3_mid": s3_mid,
    ...
    def split_params(self, params: np.ndarray):
        ...
        idx = 0
        rb1 = {c: beliefs[idx + i] for i, c in enumerate(COMBOS1)}; rw1 = {c: weights[idx + i] for i, c in enumerate(COMBOS1)}; idx += len(COMBOS1)
        rb2 = {c: beliefs[idx + i] for i, c in enumerate(COMBOS2)}; rw2 = {c: weights[idx + i] for i, c in enumerate(COMBOS2)}; idx += len(COMBOS2)
        rb3 = {c: beliefs[idx + i] for i, c in enumerate(COMBOS3)}; rw3 = {c: weights[idx + i] for i, c in enumerate(COMBOS3)}; idx += len(COMBOS3)
        rb_m = {c: beliefs[idx + i] for i, c in enumerate(COMBOS_M)}; rw_m = {c: weights[idx + i] for i, c in enumerate(COMBOS_M)}
...
# =============================================================================
# 5. 参数修复：单调性 + 边界 (针对新的 COMBOS 维度自动适配)
# =============================================================================
...
def main():
    print("=" * 84)
    print("E1-V3 消融实验：无不确定性传播 (No Uncertainty Propagation)")
    print("=" * 84)
    ...
    print("\n" + "=" * 84)
    print("E1-V3 测试集性能 (无 Global_unc)")
    print("=" * 84)
    ...
    dim_info = {
        "version": "E1-V3 No Uncertainty Propagation",
    ...
    print(f"\n✅ E1-V3 运行完毕。参数已保存至: {PARAMS_OUTPUT}")
